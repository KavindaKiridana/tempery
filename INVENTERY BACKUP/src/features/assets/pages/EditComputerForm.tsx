...
import { Grid, CircularProgress, Button, Alert } from "@mui/material";
...
function EditComputerForm() {
  ...
  const saveCommonData = async (): Promise<boolean> => {
    ...
    try {
      ...
      const error = await updateAsset(updateData as unknown as CreateCommonAssetRequest);
      if (error) {
  ...
  const handleAddAsset = async () => {
    ...
    if (ok) navigate("/viewAsset");
  };

  const goBack = () => navigate("/viewAsset");
